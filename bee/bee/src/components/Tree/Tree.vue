<template>
  <ul :class="treeCls">
    <li v-for="(item, index) in data" :key="index" :class="{[`fd-tree-treenode-disabled`]: item.disabled,[dropOverCls]: dragOverIndex === index,'filter-node': item.filter}" @dragover="dragover" @drop="drop(index,$event)" @dragenter="dragenter(index,$event)" @dragleave="dragleave(index,$event)" ref="node">
      <span :class="[`fd-tree-switcher`,{[`fd-tree-switcher-disabled`]: item.disabled,[`fd-tree-switcher-noop`]: item.isLeaf,[`fd-tree-switcher_${item.expanded ? 'open' : 'close'}`]: !item.isLeaf}]" @click="setExpand(item.disabled, index)"></span>
      ...
      <span :title="item.title" :class="selectHandleCls(item)" @click.prevent="setSelect(item.disabled, index)" :draggable="draggable" @dragstart="dragstart(index,$event)" @dragend="dragend">
        <span :class="`fd-tree-iconEle fd-tree-icon_loading fd-tree-icon__open`" v-if="item.loading"></span>
        <span :class="prefixCls + '-title'" v-html="item.title"></span>
      </span>
      <transition name="slide-up">
        <tree v-if="!item.isLeaf" :prefix-cls="prefixCls" :data="item.children" :clue="`${clue}-${index}`" :multiple="multiple" :checkable="checkable" :class="`fd-tree-child-tree-open`" v-show="item.expanded" :draggable="draggable" :async="async"></tree>
      </transition>
    </li>
  </ul>
</template>
...
.fd-tree {
  ...
  li {
    ...
    &.drag-over-gap-top {
      > span[draggable] {
        border-top-color: #d2eafb;
      }
    }
    &.drag-over-gap-bottom {
      > span[draggable] {
        border-bottom-color: #d2eafb;
      }
    }
    &.filter-node {
      > span {
    ...
    span {
      ...
      &.fd-tree-switcher {
        &.fd-tree-switcher-noop {
          cursor: default;
        }
        &.fd-tree-switcher_close {
          &:after {
  ...
  &-treenode-disabled {
    > span,
    > .fd-tree-node-content-wrapper,
    > .fd-tree-node-content-wrapper span,
    > span.fd-tree-switcher {
      color: #eee;
      cursor: not-allowed;
    }
    > .fd-tree-node-content-wrapper:hover {
